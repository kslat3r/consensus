<td title="<%= scoring_band.name %>" class="centre score"><span class="<%= scoring_band.class_name %> band"><a href="#" class="classification_detail" title="Classification details" id="search_result_<%= search_result.mongoId() %>"><%= search_result.get('score') %></a></span></td>
<% if (selected == 'item') { %>
    <td class="value">
...
<td class="centre date"><%= Util.parseDate(search_result.get('source_date_created').sec) %></td>
